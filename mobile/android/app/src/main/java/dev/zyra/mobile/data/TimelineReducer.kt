...
object TimelineReducer {
    ...
    fun apply(state: SessionView, envelope: JSONObject, force: Boolean = false): SessionView {
        ...
        val raw = JSONObject(event.toString()).apply { stamp?.let { put("_mobileCreatedAt", it); put("_mobileUpdatedAt", it) } }.toString()
        ...
        when (type) {
            ...
            "agent_end", "zyra_server_turn_completed" -> {
                ...
                running = false
                items = items.map { if (it.kind == "stream") it.copy(kind = "message") else it }
                if (event.optString("outcome") == "failed") items = items + TimelineItem(eventId, "system", event.optString("errorMessage", "The turn failed."), "error")
            }
            "message_start" -> {
                ...
                if (role == "user") items = items.filterNot { it.pending && it.role == "user" && it.text == content }
                val id = messageKey(msg) ?: eventId
                if (role != "toolResult") items = items.filterNot { it.id == id } + TimelineItem(id, role, content, if (role == "assistant") "stream" else "message", raw, reasoning = parts.reasoning, toolNames = parts.tools)
            }
            ...
            "message_update", "message_end" -> {
                ...
                val index = items.indexOfLast { (key != null && it.id == key) || (!event.optBoolean("canonicalCommit") && it.kind == "stream" && it.role == role) }
                ...
                val item = TimelineItem(key ?: previous?.id ?: eventId, role, content, if (role == "toolResult") "tool" else if (type == "message_end") "message" else "stream", retainActionStart(raw, previous), reasoning = parts.reasoning, toolNames = parts.tools,
        ...
        if (event.has("deferred") && !event.has("message") && !type.startsWith("tool_execution_")) items = items + TimelineItem(eventId, "tool", "Large output — tap to load", "deferred", raw)
    ...
    fun decode(body: String): SessionView {
        ...
        return SessionView(value.getString("id"), value.optLong("sequence"), (0 until array.length()).map {
