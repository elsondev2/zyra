...
export function MobileDeviceAccessDialog({ device, api, onClose, onSaved }: { device: MobileDevice; api: MobileAccessApi; onClose: () => void; onSaved: (state: MobileAccessState) => void }) {
    ...
    return <SettingsDialog open title="Project access" onClose={close} className="!max-w-[560px]" contentClassName="!space-y-4"
        ...
        <div className="flex items-center gap-3"><Smartphone size={19} className="shrink-0" /><div className="min-w-0"><div className="text-sm font-medium"><SensitiveSettingValue value={device.name} label="Device name" /></div><p className="mt-0.5 text-xs text-[var(--settings-text-secondary)]">These choices apply only to this device.</p></div></div>
        ...
        <SettingsExpander open={advanced} contentClassName="pt-4"><p className="border-t border-[var(--settings-border)] pt-3 text-xs leading-5 text-[var(--settings-text-secondary)]">Project visibility controls chats and their files. Terminal commands still run with your PC’s permissions. Hiding a project cannot erase copies already saved on this device.</p></SettingsExpander>
