...
function AgentDetail({ run, transcript, loading, onLoadOlder }: { run: AgentRunState; transcript: AgentTranscriptPage | null; loading: boolean; onLoadOlder: () => void }) {
    return <section className="mt-2 rounded-lg border border-white/[0.06] bg-black/10 p-3 text-[9px] leading-4 text-sparkle-text-muted/70">
        ...
        <dl className="mt-1 grid grid-cols-[62px_1fr] gap-x-2"><dt>Model</dt><dd>{run.selectedModel}{run.modelRoute?.fallback ? ` · fallback: ${run.modelRoute.fallbackReason}` : ''}</dd><dt>Effort</dt><dd>{run.effort}</dd><dt>Tools</dt><dd>{run.grantedTools.join(', ') || 'none'}</dd>{run.controlLease ? <><dt>Control</dt><dd>{run.controlLease.state} · {run.controlLease.capabilities.join(', ')}</dd><dt>Target</dt><dd className="break-all">{run.controlLease.targetId}</dd><dt>Expires</dt><dd>{run.controlLease.expiresAt}</dd></> : null}<dt>Isolation</dt><dd>{run.isolation}</dd><dt>Worktree</dt><dd className="break-all">{run.worktree?.directory || 'none'}</dd><dt>Result</dt><dd>{run.result?.text || run.error?.message || 'pending'}</dd></dl>
