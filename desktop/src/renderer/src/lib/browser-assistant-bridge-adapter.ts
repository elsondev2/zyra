...
export function createBrowserAssistantBridgeAdapter(): DevScopeApi['assistant'] {
    ...
    return {
        ...
        createPluginChat: async () => ({ success: false as const, error: 'Start Plugin Chats in Zyra Desktop.' }),
        inspectLocalPlugin: async () => ({ success: false as const, error: 'Local Plugin packages can be inspected only in Zyra Desktop.' }),
        installInspectedPlugin: async () => ({ success: false as const, error: 'Local Plugin packages can be installed only in Zyra Desktop.' }),
