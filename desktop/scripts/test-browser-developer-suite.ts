...
assert.match(browserWebview, /const emitPatch = \(patch: BrowserStatePatch\)/, 'navigation events can update known state without synchronous guest reads')
assert.match(browserWebview, /const reportSettledNavigation = \(patch: BrowserStatePatch\)[\s\S]*readNavigation\(\)/, 'back-forward capability is read only at settled load boundaries')
const liveNavigationHandler = browserWebview.slice(browserWebview.indexOf('const handleNavigate'), browserWebview.indexOf('const handleTitle'))
assert.equal(liveNavigationHandler.includes('readNavigation()'), false, 'did-navigate does not synchronously query a busy guest process')
