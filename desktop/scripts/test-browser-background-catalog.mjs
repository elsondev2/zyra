...
assert.match(pickerSource, /bottom-3 right-3 top-3[\s\S]*w-\[min\(440px,calc\(100%-24px\)\)\]/, 'Backgrounds matches History placement while remaining inside narrow Browser panes')
assert.match(pickerSource, /grid-template-columns:repeat\(auto-fit,minmax\(min\(100%,170px\),1fr\)\)/, 'the gallery uses larger previews and collapses to one column in narrow panes')
assert.doesNotMatch(pickerSource, /backdrop-blur|bg-slate-950|max-h-\[470px\]/, 'the photo canvas stays visible without a modal scrim or short centered dialog')
...
assert.doesNotMatch(pickerSource, /bg-gradient-to-t|text-white/, 'photo labels sit below images and retain theme contrast')
assert.match(pickerSource, /closing \? 'translate-x-\[calc\(100%\+16px\)\]'/, 'the picker exits toward the browser edge like History')
assert.match(pickerSource, /<footer[\s\S]*aria-label="Background rotation"/, 'rotation belongs in the stable footer instead of crowding gallery controls')
