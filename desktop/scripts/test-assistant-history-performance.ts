...
const snapshot = createAssistantLongHistoryFixture(1000, 4096)
...
const projectionRowsReadMs = performance.now() - projectionRowsStart
const projectionRowsPayloadBytes = JSON.stringify(projectionRows).length
...
assert.equal(projectionRows.activities.every((activity) => activity.payload === undefined), true, 'canonical reconciliation must not materialize historical activity payloads')
assert.ok(projectionRowsPayloadBytes < fullPayloadBytes / 2, 'canonical reconciliation uses lightweight identity rows')
