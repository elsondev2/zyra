...
const onboardingChromeSource = readFileSync(path.join(desktopRoot, 'src/renderer/src/onboarding/OnboardingChrome.tsx'), 'utf8')
const previewChromeSource = readFileSync(path.join(desktopRoot, 'src/renderer/src/components/ui/file-preview/PreviewWindowChrome.tsx'), 'utf8')
...
assert.match(quickPreviewSource, /<PreviewWindowControls\s*\/>/, 'quick preview uses the shared platform-aware window controls')
assert.match(previewChromeSource, /if \(!policy\.customWindowControls\) return null/)
assert.match(onboardingChromeSource, /policy\.customWindowControls/)
assert.match(onboardingChromeSource, /policy\.reserveMacTrafficLights/)
assert.doesNotMatch(onboardingChromeSource, /isElectronRendererRuntime/)
