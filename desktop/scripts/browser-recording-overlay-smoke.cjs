...
const deadline = setTimeout(() => { console.error('Native recording overlay fixture timed out'); app.exit(1) }, 25_000)
...
app.whenReady().then(async () => {
    ...
    const state = {
        target: { guestWebContentsId: guest.webContents.id, tabId }, status: 'recording', title: 'Native recording fixture', elapsedMs: 1500,
    ...
    await waitFor(async () => await overlay.webContents.executeJavaScript('!document.getElementById("pause").disabled'))
