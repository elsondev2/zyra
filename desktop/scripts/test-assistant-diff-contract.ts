...
assert.equal(panelSource.includes("label: browserTab?.title || 'New tab'"), true, 'each Browser page owns a titled outer Inspector tab')
...
assert.equal(panelSource.includes("kind: 'browser'; browserTabId: string"), true, 'outer Inspector state identifies Browser pages individually')
...
assert.equal(browserWorkspaceSource.includes('onTabsChange(workspaceState)'), true, 'Browser publishes page title, favicon, audio, and loading state through one retained model')
...
assert.equal(browserWebviewSource.includes("visibility: 'visible'"), true, 'retained Browser guests stay mounted while the parent viewport owns presentation')
...
assert.equal(panelSource.includes("{ id: 'browser', label: 'Browser'"), true, 'Browser is available from the workspace picker')
...
assert.equal(panelSource.includes("{ id: 'control', label: 'Control'"), true, 'Control is available from the workspace picker')
...
assert.equal(fileActionsMenuSource.includes("const resolvedMenuWidth = compact ? 176 : 180"), true, 'compact workspace menus match T3’s 11rem popup width')
assert.equal(fileActionsMenuSource.includes("? 'min-h-7 rounded-sm px-2 py-1 text-[12px] leading-none'"), true, 'compact workspace rows match T3’s 28px desktop rhythm')
...
assert.equal(inspectorSource.includes('bg-[var(--surface-inspector-tab)] text-sparkle-text'), true, 'the active workspace tab uses the theme-derived active-tab surface')
...
assert.equal(inspectorSource.includes('overscroll-x-contain px-2'), true, 'the merged tab row uses restrained title-bar padding')
...
assert.equal(indexCssSource.includes(".drag-region [draggable='true']"), true, 'draggable Inspector tabs opt out of native window dragging')
assert.equal(titleBarSource.includes("assistantWorkspaceActive && 'absolute right-0 top-0 z-[3]'"), true, 'window controls overlay their reserved part of the merged Inspector title bar')
...
assert.equal(titleBarSource.includes('? `${sidebarWidthRef.current}px`'), true, 'the expanded title-bar app zone follows the live local sidebar width')
assert.equal(titleBarSource.includes("? '112px'\n                        : expandedAssistantSidebar"), true, 'loading immediately returns the title bar to its compact app zone')
assert.equal(titleBarSource.includes("assistantWorkspaceActive && !loadingScreenActive && 'border-r border-[var(--surface-panel-divider)]'"), true, 'the title-bar sidebar divider is absent while loading')
...
assert.equal(chatRailSource.includes('pointer-events-auto fixed bottom-0 left-0 top-[34px] z-[59] w-6'), true, 'the left-edge hover target remains reachable above chat content')
