...
assert.equal(panelSource.includes("activeWorkspaceTab?.kind === 'browser' ? 'flex min-h-0 flex-1' : 'hidden'"), true, 'Browser stays mounted while other Inspector tabs are active')
assert.equal(panelSource.includes('statusIcon: browserAudible ? <Volume2'), true, 'the outer Browser workspace tab marks retained site audio')
...
assert.equal(newTabSource.includes('grid-cols-3'), true, 'the new-tab chooser uses a three-by-two tile grid')
...
assert.equal(newTabSource.includes("label: 'Subagents'"), true)
...
assert.equal(newTabSource.includes('max-w-[306px]'), true, 'the compact three-by-two chooser is centered instead of filling the page')
...
assert.equal(rendererCssSource.includes('.assistant-turn-review__conversation-pane,\n.assistant-turn-review__narrow-panel'), true, 'the context surface itself can shrink within its grid or flex column instead of being clipped')
