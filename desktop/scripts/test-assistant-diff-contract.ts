...
assert.equal(filesWorkspaceSource.includes('active && preview.previewFile'), true, 'inactive Files tabs hide their modal without losing preview state')
...
assert.equal(panelSource.includes("label: browserTab?.url ? browserTab.title || 'New tab' : 'New tab'"), true, 'blank Browser pages always own the literal New tab title')
...
assert.equal(resourcesWorkspaceSource.includes('aria-rowindex={range.start + rowOffset + 2}'), true, 'virtualized resource rows expose their absolute table position')
...
assert.equal(resourcesWorkspaceSource.includes('entry.id !== \'all\' && filterCounts[entry.id] > 0'), true, 'empty resource filter tags stay hidden')
...
assert.equal(browserViewManagerSource.includes('session: getGlobalBrowserSession()'), true, 'main creates pages only in the exact global Browser session')
...
assert.equal(fileActionsMenuSource.includes("? 'min-h-8 rounded-md px-2 py-1.5 text-[11px] leading-none'"), true, 'compact workspace rows retain the readable 32px menu rhythm')
...
assert.equal(inspectorSource.includes('width: targetWorkspaceTabWidth'), true, 'new tabs mount directly at the final shared width')
