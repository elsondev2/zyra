...
assert.deepEqual(openedBrowserSessionModes, ['normal'], 'agent-opened Browser tabs use the documented normal profile by default')
...
assert.deepEqual(openedBrowserSessionModes, ['normal', 'normal'], 'agents can explicitly request the normal profile')
await tool.execute('visual-open-private-tab', { operation: 'open_tab', reveal: true, sessionMode: 'incognito' })
assert.deepEqual(openedBrowserSessionModes, ['normal', 'normal', 'incognito'], 'incognito remains an explicit private-storage choice')
...
const agentUseOverlaySource = readFileSync(new URL('../src/renderer/src/pages/assistant/AssistantAgentUseOverlay.tsx', import.meta.url), 'utf8')
const agentUseOverlayStyles = readFileSync(new URL('../src/renderer/src/pages/assistant/AssistantAgentUseOverlay.css', import.meta.url), 'utf8')
...
assert(panelSource.includes('<InspectorWorkspaceSurface kind="browser"'), 'Browser uses the shared retained workspace container')
const inspectorSurfaceSource = readFileSync(new URL('../src/renderer/src/pages/assistant/InspectorWorkspaceSurface.tsx', import.meta.url), 'utf8')
assert(inspectorSurfaceSource.includes('browser: { retainGeometry: true }'))
assert(inspectorSurfaceSource.includes('pointer-events-none invisible absolute inset-0 flex'), 'inactive Browser geometry stays mounted without accepting input')
