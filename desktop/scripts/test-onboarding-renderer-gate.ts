...
const footerSource = readFileSync(resolve(here, '../src/renderer/src/onboarding/OnboardingFooter.tsx'), 'utf8')
const stepsSource = readFileSync(resolve(here, '../src/renderer/src/onboarding/OnboardingSteps.tsx'), 'utf8')
const stageSource = readFileSync(resolve(here, '../src/renderer/src/onboarding/OnboardingStage.tsx'), 'utf8')
const appearanceSource = readFileSync(resolve(here, '../src/renderer/src/onboarding/AppearanceStep.tsx'), 'utf8')
const signInSource = readFileSync(resolve(here, '../src/renderer/src/onboarding/ConnectOpenAiStep.tsx'), 'utf8')
...
assert.match(footerSource, /Setup step \$\{content\.index \+ 1\} of \$\{content\.total\}/, 'compact progress must describe the current numbered step')
assert.match(flowSource, /<OnboardingStage[\s\S]*onboarding-step-heading/, 'setup pages use the shared animated stage')
assert.match(stageSource, /onboarding-step-scroll[\s\S]*onboarding-step-content/, 'step titles and choices share one centered scrolling unit')
assert.match(footerSource, /onboarding-action-dock/, 'Back and Continue must share a stable viewport dock')
assert.match(footerSource, /onboarding-dock-progress/, 'numbered progress must stay centered between the fixed actions')
...
assert.match(stageSource, /scroll\.scrollTop = 0/, 'each centered step must open at the top of its own scroll region')
assert.match(motionSource, /\.onboarding-step-scroll\s*\{[\s\S]*?display: flex;/)
assert.match(motionSource, /\.onboarding-action-dock\s*\{[\s\S]*?position: fixed;/)
assert.match(motionSource, /\.onboarding-step-content\s*\{[\s\S]*?margin-block: auto;[\s\S]*?padding-block: 32px;/, 'setup centers short content and preserves scrollable headroom for tall steps')
...
assert.match(signInSource, /Connect a model provider so Zyra can answer questions and carry out tasks/, 'sign-in explains why a provider connection is needed')
assert.match(signInSource, /Use an API key/)
assert.match(signInSource, /inert=\{!apiKeyOpen\}/, 'collapsed key entry stays out of keyboard navigation')
assert.match(signInSource, /<OpenAiLogo/, 'the primary ChatGPT action must use the theme-aware OpenAI mark')
assert.match(openAiLogoSource, /fill="currentColor"/, 'the OpenAI mark must follow the active Zyra theme')
assert.match(stepsSource, /onboarding-review-grid[\s\S]*onboarding-review-detail/, 'the final review keeps the account, appearance and folder summary scannable')
...
const welcomeStepSource = stepsSource.split('export function WelcomeStep')[1]?.split('export { ConnectOpenAiStep }')[0] || ''
