...
assert.match(flowSource, /analyticsChoice !== null/, 'opening Zyra requires an explicit analytics choice')
assert.match(stepsSource, /WelcomeStep[\s\S]*analyticsProps\.analyticsChoice === null/, 'new installs choose analytics before setup events begin')
assert.match(stepsSource, /role="radiogroup" aria-label="Product analytics preference"/, 'analytics consent is a clear standalone onboarding decision')
assert.match(stepsSource, /Share diagnostics and usage/, 'the opt-in action states what data it enables')
assert.match(stepsSource, /No thanks[\s\S]*Keep product analytics off/, 'declining analytics is a complete first-class path')
assert.match(stepsSource, /never sends prompts, responses, transcripts, files, paths, URLs, account identity, or terminal content/, 'the consent copy names the sensitive data boundary')
