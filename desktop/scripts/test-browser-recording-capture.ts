...
broker.armPresentation(owner, guest)
assert.equal(broker.hasRecording(owner), false, 'menu presentation cannot grant microphone access or create a recording')
assert.throws(() => broker.arm(owner, guest, 'tab'), /already|Another Browser capture/, 'a second purpose cannot replace a pending grant')
broker.cancel(owner.id)
assert.equal(broker.hasGrant(owner), true, 'stopping a recorder cannot cancel a pending menu presentation')
assert.deepEqual(request(frame, true), {}, 'menu presentation can never capture tab/system audio')
assert.deepEqual(request(frame), { video: guestFrame })
broker.arm(owner, guest, 'tab')
broker.cancelPresentation(owner.id, guest.id)
assert.equal(broker.hasGrant(owner), true, 'closing a menu cannot cancel a recorder audio grant')
assert.deepEqual(request(frame, true), { video: guestFrame, audio: guestFrame, enableLocalEcho: true })
broker.armPresentation(owner, guest)
broker.cancelPresentation(owner.id, 99)
assert.equal(broker.hasGrant(owner), true, 'a stale different-tab cleanup cannot cancel the current preview')
broker.cancelPresentation(owner.id, guest.id)
assert.equal(broker.hasGrant(owner), false)
assert.equal(broker.hasRecording(owner), true, 'preview cleanup preserves an existing recording')
broker.cancel(owner.id)
