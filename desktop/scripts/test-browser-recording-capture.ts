...
broker.arm(owner, guest, 'tab')
assert.throws(() => broker.arm(owner, guest, 'off'), /Another Browser capture/, 'a second request cannot replace an outstanding recording grant')
assert.equal(broker.hasGrant(owner), true)
assert.deepEqual(request(frame, true), { video: guestFrame, audio: guestFrame, enableLocalEcho: true })
assert.equal('armPresentation' in broker, false, 'app overlays no longer expose a display capture entry point')
