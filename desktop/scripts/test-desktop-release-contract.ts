...
assert.match(allowlistedTuiArchiveSource, /const include = \[[\s\S]*'bin',[\s\S]*'src',[\s\S]*'prompts'/, 'the canonical TUI archive is built from an explicit runtime allowlist')
