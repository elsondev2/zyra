...
const directory = await mkdtemp(join(tmpdir(), 'zyra-approval-ui-'))
const success = 'Approval panel interactions: exact decisions, explicit chat scope, keyboard, busy lock and queue lifecycle: ok'
try {
    await build({
        entryPoints: [join(desktop, 'scripts/fixtures/approval-panel-interactions.tsx')],
...
let check;
const timeout=setTimeout(()=>{console.error('Approval fixture deadline');app.exit(1)},12000);
