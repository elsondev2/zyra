...
assert.match(transcriptionSource, /chatgpt-account\.mjs/u, 'ChatGPT transcription must load the Electron-safe account module')
...
assert.match(transcriptionSource, /resolveChatGptAccountAuth/u, 'ChatGPT transcription must share the same Pi account source as account usage')
...
assert.match(recorderBarSource, /<VoiceWaveform levels=\{waveformSamples\} processing/u, 'transcription should retain a compact animated waveform beside its status')
assert.match(recorderBarSource, /bg-\[var\(--color-text-secondary\)\] opacity-80/u, 'waveform bars must use a generated CSS-variable background utility')
...
assert.match(rendererCssSource, /@keyframes assistant-voice-wave-processing/u, 'the transcription waveform should have a dedicated processing animation')
assert.match(rendererCssSource, /prefers-reduced-motion: reduce[\s\S]{0,180}\.assistant-voice-wave-processing/u, 'the processing waveform should stop animating for reduced motion')
...
assert.match(composerSource, /<AnimatedHeight isOpen=\{!showCodexRecorder\} duration=\{220\}>/u, 'the text-entry area should animate between composer and recorder states')
assert.match(composerSource, /showCodexRecorder[\s\S]{0,120}\? 'gap-2 px-2 py-2'/u, 'the active recorder should use one compact composer row')
