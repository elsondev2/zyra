...
assert.equal(virtualTimelineSource.includes('const preserveAnchor = () =>'), true, 'anchor preservation spans deferred disclosure content mounting')
assert.equal(virtualTimelineSource.includes("scrollElement.scrollBy({ top: delta, behavior: 'auto' })"), true, 'bounded corrections keep the disclosure trigger in place after layout')
