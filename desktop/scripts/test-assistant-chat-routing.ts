...
assert.equal(connectedRailSource.includes('await railController.selectSession'), false, 'route navigation must be the single selection trigger so browser history does not receive duplicate chat entries')
assert.equal(connectedRailSource.includes('await railController.selectThread'), false, 'thread routes must not race a second direct selection')
...
assert.equal(assistantStoreSource.includes("window.devscope.assistant.connect({ sessionId: input.sessionId })"), true, 'thread routes must reconnect the selected session so Back and Forward remain usable')
