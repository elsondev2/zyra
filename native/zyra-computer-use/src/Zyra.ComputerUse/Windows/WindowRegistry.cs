    ...
    public WindowCandidate[] ListVisibleWindows()
    {
        ...
        NativeMethods.EnumWindows((handle, _) =>
        {
            ...
            try
            {
                ...
                results.Add(new WindowCandidate(token, title, process.ProcessName, identity, process.Id, blockedReason is not null, blockedReason));
            }
            catch { }
            return results.Count < 256;
        }, nint.Zero);
        return results.OrderBy(entry => entry.ApplicationName).ThenBy(entry => entry.Title).ToArray();
    }
