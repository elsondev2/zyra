...
export class ZyraComponentHost {
  ...
  render(options = {}) {
    ...
    if (this.shouldRenderFixedOnly(options, width, resized)) {
      ...
      const fixedOutput = fixedLines.join("\n");
      if (fixedOutput === this.lastFixedOutput && contentLines.length === this.renderedContentLines.length) {
        this.renderCursorOnly(cursorTarget);
        return;
      }
    ...
    if (!resized && contentOutput === this.lastContentOutput && fixedOutput === this.lastFixedOutput) {
      this.renderCursorOnly(cursorTarget);
      return;
    }
  ...
  diffRender(lines, options = {}) {
    ...
    if (firstChanged === -1) {
      this.renderCursorOnly(options.cursorTarget);
      return;
    }
  ...
  renderCursorOnly(cursorTarget) {
    if (cursorTargetsEqual(this.currentCursorTarget, cursorTarget)) return;
    const movement = this.currentCursorTarget
      ? renderCursorMovementFrom(this.currentCursorTarget, cursorTarget)
      : renderCursorMovement(this.renderedLines.length - 1, cursorTarget);
    if (!movement) return;
    this.beginBatch();
    this.output.write(movement);
    this.endBatch(Boolean(cursorTarget));
    this.currentCursorTarget = cursorTarget ?? null;
  }
...
function cursorTargetsEqual(left, right) {
